    ...
    <div className="flex flex-col space-y-4">
      {grupos.map(({ title, icon, total, p1, p2 }, idx) => (
        <div key={idx} className="grid grid-cols-[1fr_0.6fr] gap-3 items-start">
          ...
          <div className="bg-primary/10 min-w-[80px] shadow-sm w-fit h-fit rounded-sm py-4.5 flex flex-col items-center">
          ...
          <div className="flex flex-col space-y-3">
            {[{ label: 'P1', value: p1 }, { label: 'P2', value: p2 }].map(({ label, value }, subIdx) => (
              <div key={subIdx} className="bg-neutral/10 min-w-[65px] shadow-sm w-fit h-fit rounded-sm py-1.5 px-4 flex flex-col items-center">
